"""Mouth eligibility using the configured landmark backend or explicit Haar fallback."""
...
def _landmark_result(frame) -> MouthValidation:
    from landmark_provider import get_landmark_provider
    result = get_landmark_provider().detect(frame)
    return MouthValidation(
        "ok" if result.mouth_visible else "no_mouth",
        result.provider,
        result.face_count,
        result.mouth_visible,
        result.confidence,
        result.reason,
        None,
        result.landmarks,
...
def _haar_result(frame) -> MouthValidation:
    try:
        import cv2
    except ImportError:
        return MouthValidation("unavailable", "opencv-haar-mouth", 0, False, 0.0, "OpenCV is not installed.")
    ...
    if not faces:
        return MouthValidation("no_face", "opencv-haar-mouth", 0, False, 0.0, "No frontal face detected.")
    ...
    mouths = [] if lower.size == 0 else list(mouth_cascade.detectMultiScale(
        lower, scaleFactor=1.1, minNeighbors=8,
        minSize=(max(16, int(w * 0.16)), max(8, int(h * 0.08)))
    ))
    if not mouths:
        return MouthValidation("no_mouth", "opencv-haar-mouth", len(faces), False, 0.0,
                               "Face detected but mouth region was not confidently detected.")
    mx, my, mw, mh = max(mouths, key=lambda b: int(b[2]) * int(b[3]))
    box = (x + int(mx), y + int(h * 0.45) + int(my), int(mw), int(mh))
    ratio = (int(mw) * int(mh)) / max(1, w * h)
    confidence = min(1.0, max(0.0, ratio / 0.035))
    suitable = confidence >= 0.45
    return MouthValidation("ok" if suitable else "small_mouth", "opencv-haar-mouth", len(faces), suitable,
                           round(confidence, 3),
                           "Mouth candidate detected in the primary face." if suitable else "Mouth candidate is too small for safe animation.", box)
...
def validate_frame(frame) -> MouthValidation:
    ...
    if provider == "mediapipe":
        return _landmark_result(frame)
    if provider in {"opencv-haar-mouth", "opencv"}:
        return _haar_result(frame)
    return MouthValidation("not_configured", provider, 0, False, 0.0,
                           "No mouth validation provider is configured.")
