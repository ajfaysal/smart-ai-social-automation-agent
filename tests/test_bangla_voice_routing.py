...
def test_gender_routes_use_distinct_profiles():
    ...
    assert routes["S01"].voice_profile in {
        "bn_c01_f_young", "bn_c03_f_adult", "bn_c05_f_mature",
        "bn_c07_f_soft", "bn_c09_f_energetic",
    }
    assert routes["S02"].voice_profile in {
        "bn_c02_m_young", "bn_c04_m_adult", "bn_c06_m_mature",
        "bn_c08_m_deep", "bn_c10_m_energetic",
    }
...
def test_valid_reference_audio_is_preferred(tmp_path: Path):
    ...
    turns = (SpeakerTurn("S01", 0, 2),)
    identities = {"S01": CharacterIdentity("C01", "S01", 0.95, str(reference), "female")}
    routes = build_voice_routes(turns, identities)
    assert routes["S01"].reference_audio == str(reference)
    assert routes["S01"].voice_source == "reference_audio"
    assert routes["S01"].voice_profile is None
