...
def _make_fixture(path: Path, duration: float = 2.0) -> None:
    subprocess.run(
        ["ffmpeg", "-y", "-v", "error", "-f", "lavfi", "-i", "color=c=black:s=320x180:r=24", "-f", "lavfi", "-i", "sine=frequency=440:sample_rate=48000", "-t", str(duration), "-c:v", "libx264", "-pix_fmt", "yuv420p", "-c:a", "aac", "-shortest", str(path)],
        check=True,
    )
...
def test_real_media_qc_boundary():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        _make_fixture(media)
        shots = [{"start": 0.0, "end": 1.0, "index": 0}, {"start": 1.0, "end": 2.0, "index": 1}]
        validate_shot_plan(shots, 2.0)
        validate_reassembled(media, 2.0, shots)
        manifest = [{"index": 0, "start": 0.0, "end": 0.8, "drift_ms": 0.0}, {"index": 1, "start": 1.0, "end": 2.0, "drift_ms": 5.0}]
