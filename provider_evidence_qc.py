...
def ingest_provider_evidence(evidence_map: dict[str, Any]) -> None:
    """Validate all external evidence before committing any runtime records."""
    ...
    for provider, evidence in evidence_map.items():
        ...
        if state == ProviderState.SUCCEEDED.value:
            ...
            pending.append(succeeded(provider, reason or "Provider execution succeeded.", version=version, capabilities=capabilities))
            continue
        ...
        if state == ProviderState.FAILED.value:
            ...
            pending.append(failed(provider, reason or "Provider execution failed.", version=version, capabilities=capabilities))
            ...
            pending.append(attempted(provider, reason or "Provider execution started.", version=version, capabilities=capabilities))
            ...
            pending.append(configured(provider, reason or "Provider is configured and ready.", version=version, capabilities=capabilities))
            ...
            pending.append(unavailable(provider, reason or "Provider is unavailable.", version=version))
            ...
            pending.append(skipped(provider, reason or "Provider execution skipped.", version=version))

    # No runtime state changes occur until every entry has passed validation.
